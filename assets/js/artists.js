// Artist registry — keyed by lineup display name.
// Source: PLX Ode Bokningsformuläret sheet (GAMLA + NYA tabs).
// Bios are intentionally short. `web` falls back to instagram in panel/banner UI.

export const ARTISTS = {
  "Alexis": {
    name: "Alexis",
    bio: "Alexis, aka azar, is a Malmö based artist and the person behind the label blundar. During PLX ODE JOY Alexis will present an hour of ambient related to the blundar sound palette — full of experimental and immersive sounds.",
    web: "https://blundar.co/",
    instagram: "blundar.co",
  },
  "Amina Szecsödy": {
    name: "Amina Szecsödy",
    bio: "Amina Szecsödy (b. 1995) works across theatre, choreography, and visual arts. Her work engages text, sound and the body to draw on elements of pop culture, newspaper articles, cinema and mythmaking. She edits, cuts and pastes dissonant voices and phenomena of the past and the future into the immediate present.",
  },
  "Cecilia Sterner": {
    name: "Cecilia Sterner",
    bio: "Cecilia Sterner is a visual artist based in Malmö. She works with installation and performance and lures you into a playful world where the ugly, raw, dark, sophisticated and ludicrous meet — because the world is full of contradictions.",
    instagram: "ceciliasterner",
  },
  "Chris Shields": {
    name: "Chris Shields",
    bio: "Chris Shields is a musician and developer of instruments for scientific and musical experimentation. He splits time between touring and recording music alone and with others (in the duo Vid Edda) and involvement in arctic climate research with the Danish government.",
    instagram: "lucky.goldstar",
  },
  "DINA": {
    name: "DINA",
    bio: "DINA is a Berlin-based producer and DJ and co-founder of Nachtcrew — a label and party series fusing rave sounds, art and 3D mapping into immersive sensorial experiences. Her production spans spacey ambience, punching basslines, and ethereal trance.",
    web: "https://soundcloud.com/dinakhashan",
    instagram: "dinakhashan",
  },
  "Eli Frankel": {
    name: "Eli Frankel",
    bio: "Swedish performer, artistic director, dramaturge, composer, and educator. Active member of art collectives such as RadioArt, Alice Collective for Sound & Stage and PLX. He curates festivals, creates new music-theatre works, and explores the intersection of performance, sound, and politics.",
    instagram: "eli_o_frankel",
    work: "IRIDAE",
    workText: "A newly composed electroacoustic opera, with music and libretto by Marika Markström in collaboration with PLX, directed by Eli Frankel and performed by singer and performance artist Hannes Ferm. Voice, song, and phonetic fragments merge with an acousmatic texture, and the work unfolds in five short acts that touch upon the balancing state of the contemporary world and the human condition through (un)conscious journeys across inner and outer environments.",
  },
  "Dmn7": {
    name: "Dmn7",
    bio: "Marika Markström is a composer, performer and audiovisual artist, graduated in electroacoustic composition from the Royal College of Music in Stockholm. Her first retrospective album, Voices of the Unheard, was released by PLX Records in 2025 under the artist name DMN7.",
    instagram: "marikamadeleine",
    work: "IRIDAE",
    workText: "A newly composed electroacoustic opera, with music and libretto by Marika Markström in collaboration with PLX, directed by Eli Frankel and performed by singer and performance artist Hannes Ferm. Voice, song, and phonetic fragments merge with an acousmatic texture, and the work unfolds in five short acts that touch upon the balancing state of the contemporary world and the human condition through (un)conscious journeys across inner and outer environments.",
  },
  "Hannes Ferm": {
    name: "Hannes Ferm",
    bio: "Artist and music producer born 1994 in Piteå, Sweden. Graduated from the bachelor program in fine arts at Konstfack in 2021, since then active in music production, composition, and performance — including a series of works around the historic and timbral world of the castrati singers.",
    instagram: "hannes_ferm",
    work: "IRIDAE",
    workText: "A newly composed electroacoustic opera, with music and libretto by Marika Markström in collaboration with PLX, directed by Eli Frankel and performed by singer and performance artist Hannes Ferm. Voice, song, and phonetic fragments merge with an acousmatic texture, and the work unfolds in five short acts that touch upon the balancing state of the contemporary world and the human condition through (un)conscious journeys across inner and outer environments.",
  },
  "Ellinor Åslund": {
    name: "Ellinor Åslund",
    bio: "Ellinor Åslund är en multimedial konstnär utbildad på Det Kongelige Danske Kunstakademi, med flera performances och utställningar i Danmark och Sverige bakom sig — bland annat på Kunsthal Charlottenborg, Wilson Saplana Gallery och Four Boxes Gallery.",
    web: "https://ellinoraslund.com",
    instagram: "elli_nor_st._aaslund",
    work: "Die Ewige Suppe",
    workText: "En surreell dockteater-opera som utspelar sig i en bar, där protagonisten — en keramisk docka modellerad efter konstnären själv — genom sånger berättar fragmenterade minnen som tar sin utgångspunkt i graviditet och moderskap. Teatern utvecklar sig som en evig soppa: en evigt föränderlig blandning av minnen, drömmar och politiska verkligheter, allt eftersom baren omvandlas till en skiftande scen där sjuksystrar, sagofigurer, byråkratiska utövare, nationalistiska gatubokhandlare och krigsspöken sammanstrålar för att kasta ljus över hur personliga och kollektiva historier sammanvävs.",
  },
  "Emil Keller Skousen": {
    name: "Emil Keller Skousen",
    bio: "Emil Keller Skousen works with a blend of sound, performance, text, drawing, and sculpture. In recent years he has toured with the performance “Mr. Szinck”, a drone-noise show featuring the lonely knight of the post-apocalypse.",
    web: "https://grillting.com/",
    instagram: "grill_ting",
  },
  "Fauna": {
    name: "Fauna",
    bio: "Det svänger och bubblar och brinner och djuren kommer ut från skogen.",
    web: "https://faunagbg.bandcamp.com/",
    instagram: "fauna_gbg",
  },
  "Francis Patrick Brady & TFK": {
    name: "Francis Patrick Brady & TFK",
    bio: "Konstnären Francis Patrick Brady skapar deltagarbaserade fiktioner. Hans projekt The Forgetful Kindred (FKK) är en gåtfull rörelse av hantverkare i Haderslev som bygger fred med händerna men drabbats av en kollektiv amnesi. De tillverkar ständigt nya objekt och ritualer vars syfte de genast glömmer.",
    web: "https://francispatrickbrady.com/",
    instagram: "francispatrickbrady",
  },
  "Frans Felix Ahlberg Eriksson": {
    name: "Frans Felix Ahlberg Eriksson",
    bio: "Felix is a visual artist and interior architect based in Stockholm. Educated at Konstfack (Bachelor of Fine Arts 2011) and Nyckelviken. Apart from his art practice he has worked in architectural offices and freelance in interior architecture and art production.",
  },
  "Jenny Palén": {
    name: "Jenny Palén",
    bio: "Jenny Palén (f. 1982 i Stockholm) tog examen från Kungliga Konsthögskolan 2015. Hennes arbete omfattar installation, video, skulptur och måleri, med fokus på existentiella och surrealistiska teman som utforskas genom film, performance och scenografi.",
  },
  "Johnny Essing": {
    name: "Johnny Essing",
    bio: "Johnny Essing Är Mitt Namn. Denna natt överlämnar jag till mina Elektroniska Ljudmaskiner, Antika Elektriska Gitarrer, Högljudda Maraccas, Okända Balalajkor, Inneboende Röster och CV-styrda Varelser att frammana både Solen & Tåren, Mjuka Knivmoln, Vems Natt, och de Ofrånkomliga Tab-Orgs.",
  },
  "Jonathan Sendborn Pohlin": {
    name: "Jonathan Sendborn Pohlin",
    bio: "Jag arbetar i ett visuellt landskap där drakar, monster och blommor samexisterar. Verken rör sig mellan det figurativa och det abstrakta, där måleri och skulptur tillsammans bygger upp en värld som känns både lekfull och oroande — naivism och expressionism, spontant och nära impulsen.",
    instagram: "Sendborn",
  },
  "Jules Reidy": {
    name: "Jules Reidy",
    bio: "Jules Reidy’s sublime music maps the human experience in glittering constellations of sound. The Berlin-based guitarist explores microtonal mysticism and alternate tunings drawn from transcendental folk and minimalism, alongside playful experiments with smudged pop melodics and sampling.",
    instagram: "jules_reidy",
  },
  "Kristoffer Grip": {
    name: "Kristoffer Grip",
    bio: "Kristoffer Grip (born 1984) lives outside of Ronneby and works with painting, vitreous enamels, drawing, writing and music in regard to stories, myths, symbols and our relationship to the immaterial and the imagined. He released his debut album “Wildest Hope” in March 2026.",
    web: "https://kristoffergrip.com",
  },
  "Mohammad Reza Mortazavi": {
    name: "Mohammad Reza Mortazavi",
    bio: "Mohammad Reza Mortazavi is a virtuoso percussionist from Iran, known for playing traditional Persian instruments such as the tombak and daf. After developing more than thirty new striking techniques he settled in Berlin; acclaimed performances have taken place at Berlin Philharmonie, Paris Pantheon, Villa Medici and Sydney Opera House.",
    instagram: "m_r_mortazavi",
  },
  "Nils Bergendal": {
    name: "Nils Bergendal",
    bio: "Nils är en gammal kär vän till PLX. Han har en särskild förkärlek för att låta stubbar och tält humma fram ljud. Efter att ha gjort succé med sin bok om en påse jordnötter kommer Nils till PLX med ett nytt verk där munnen står helt i centrum.",
    web: "https://nilsbergendal.com/",
    instagram: "nils.bergendal",
  },
  "One secret each": {
    name: "One secret each",
    bio: "Maria Bergström (b. 1993, Falun) lives and works in Stockholm. Composer and Sound Artist, graduate of Stockholm University of the Arts (2024) with works featured at Unga Klara, Orionteatern and Oper Graz. She released her debut album “Debut” as One secret each in 2025 via PLX Records.",
  },
  "Patrik Söderstam": {
    name: "Patrik Söderstam",
    bio: "Patrik Söderstam is an artist, graphic designer, costume designer and photographer. Born and educated in Sweden, he studied menswear construction at the Stockholm Cutting Academy and went on to study Menswear Fashion at MA level at Central Saint Martins College, London.",
    web: "https://www.showstudio.com/contributors/patrik_soderstam",
    instagram: "mixpatriksoderstam",
  },
  "Praktikantgruppen": {
    name: "Praktikantgruppen",
    bio: "Ett rum i ett rum i ett rum, undangömt men märkligt bekant — något du kanske vill återvända till, eller låta vara. Här finns plats för lek och vila, att stanna en stund eller dra sig undan. Praktikantgruppen har tidigare deltagit i uppbyggandet av PLX Tjärö men har nu släppts lösa.",
  },
  "Private Parts": {
    name: "Private Parts",
    bio: "Vår älskade duo som först stod på en PLX-scen för över tio år sedan är äntligen tillbaka. From Italy with love återförenas Alexandra Karpilovski och Danilo Colonna som Private Parts. It will get juicy, it will get hot, heart to heart — clubby meditation and a whole lot of sensation.",
    web: "https://m.soundcloud.com/prvtprts",
    instagram: "prvtprts_",
  },
  "Rebecca Moss": {
    name: "Rebecca Moss",
    bio: "Rebecca Moss is a UK-based artist whose work explores absurdity and precarity through sculpture, video, performance, installation, and participatory practice. She is especially inspired by slapstick performance — for its emotional expression of instability and its potential to challenge power.",
    instagram: "_rebecca.moss",
    work: "Danse Macabre",
  },
  "SMiSK": {
    name: "SMiSK",
    bio: "SMiSK firar tio år som band! Med sin uppstudsiga punk, sina glädjedödande texter och sina sjuka performance-jippon har de retat gallfeber på allt från flaggälskande Svärjevänner till svandyrkande monogamister. Stålsätt dig för att bli både ljudmässigt och visuellt arkebuserad.",
    web: "https://smisk.bandcamp.com",
  },
  "Snejina Latev": {
    name: "Snejina Latev",
    bio: "Snejina Latev is a multidisciplinary artist and filmmaker whose work moves at the intersection of documentary storytelling, commercial film, and visual art. She is also a part of the PLX community.",
  },
  "Stina Force": {
    name: "Stina Force",
    bio: "With a taste for the absurd and the strange, Fors creates solo works that move between dance, theater, music, and experimental performance. She is known for her one-woman band Stina Force, the performance A Mouthful of Tongues, and most recently the durational piece Answer Me Pythia.",
  },
  "Velvet Forever": {
    name: "Velvet Forever",
    bio: "Malcolm Jeppsson makes noisy ambient music playing on nostalgic and queer-coded themes with a contemporary, post-ironic approach. Representing the queer ambient club concept Velvet Forever, she will mix her own music with hidden gems and ambient classics.",
    instagram: "velvet__forever",
  },
  "Wes Baggaley": {
    name: "Wes Baggaley",
    bio: "It’s a no-nonsense affair with Wes Baggaley, the DJ cult favourite of raw rhythms and pulsating electronic bass. Sets are dark, deep and propulsive — a tight mix of rough house and brutal techno covering acid, EBM, industrial and new wave. Resident of queer house party Bottom Heavy at Dalston Superstore.",
    web: "https://soundcloud.com/wes-baggaley",
    instagram: "wesbaggaley707",
  },
  "Zoë Mc Pherson": {
    name: "Zoë Mc Pherson",
    bio: "Co-founder of the SFX platform with Alessandra Leone. Their immersive audiovisual performance “Is THIS real?”, based on Mc Pherson’s 2025 album Upside Down, develops a performative language where sound, image, and stage environment form one fluid system through real-time visuals, depth capture and adaptive lighting.",
    instagram: "zoemcphers",
  },
  "Alessandra Leone": {
    name: "Alessandra Leone",
    bio: "Co-founder of the SFX platform with Zoë Mc Pherson. Handles visuals and lights for their AV duo project — live camera feeds, depth capture, 3D scans and adaptive lighting reshape the space around Mc Pherson’s live presence.",
    instagram: "ale.byss",
  },
};

// Resolve to a clickable URL: web link if present, else Instagram, else null.
export function artistUrl(name) {
  const a = ARTISTS[name];
  if (!a) return null;
  if (a.web) return a.web;
  if (a.instagram) return `https://instagram.com/${a.instagram.replace(/^@/, "")}`;
  return null;
}
